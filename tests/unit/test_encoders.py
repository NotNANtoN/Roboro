...
class TestMLPEncoder:
    def test_output_shape(self, random_obs, obs_dim, hidden_dim):
        feature_dim = 64
    ...
    def test_feature_dim_property(self, obs_dim):
        encoder = MLPEncoder(obs_dim=obs_dim, feature_dim=128)
        assert encoder.feature_dim == 128

    def test_different_activations(self, random_obs, obs_dim):
    ...
    def test_layer_norm(self, random_obs, obs_dim):
        encoder = MLPEncoder(obs_dim=obs_dim, use_layer_norm=True)
        out = encoder(random_obs)
        assert out.shape[0] == random_obs.shape[0]

    def test_n_layers(self, random_obs, obs_dim):
        for n in (2, 3, 4):
            encoder = MLPEncoder(obs_dim=obs_dim, n_layers=n)
            out = encoder(random_obs)
            assert out.shape[0] == random_obs.shape[0]

    def test_gradient_flow(self, obs_dim):
