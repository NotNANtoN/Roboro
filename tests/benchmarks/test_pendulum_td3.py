"""Performance benchmarks: TD3 on Pendulum-v1.
...
Via pytest::
    ...
    pytest -m "benchmark and not slow" -k "pendulum_td3" -s   # fast only
    pytest -m benchmark -k "pendulum_td3" -s                   # both tiers

Standalone with Hydra overrides::

    python tests/benchmarks/test_pendulum_td3.py
    python tests/benchmarks/test_pendulum_td3.py train.device=mps
"""
...
from roboro.presets import TD3
from roboro.training.trainer import TrainResult

# ── shared logic ─────────────────────────────────────────────────────────────


def _run_td3_pendulum(cfg: Any) -> TrainResult:
    """Run TD3 on Pendulum-v1.  Accepts dataclass or Hydra DictConfig."""
...
try:
    ...
    def test_td3_pendulum_improves(train_overrides: dict[str, Any]) -> None:
        """TD3 should improve beyond random (> -1200) within 20k steps.

        Random Pendulum scores ~-1200 to -1500.  A working TD3 should be
        ...
        result = _run_td3_pendulum(cfg)

        assert len(result.eval_rewards) >= 2, "Need at least 2 evals"
        best_eval = max(result.eval_rewards)
        assert best_eval > -1200, (
            f"TD3 didn't improve beyond random on Pendulum-v1 in 20k steps. "
            f"Best eval: {best_eval:.0f} (need > -1200), all evals: {result.eval_rewards}"
        )

    @pytest.mark.benchmark
    @pytest.mark.slow
    def test_td3_pendulum_solves(train_overrides: dict[str, Any]) -> None:
        """TD3 should reach >= -400 on Pendulum-v1 within 50k steps."""
        cfg = replace(TD3, train=replace(TD3.train, **train_overrides))
        result = _run_td3_pendulum(cfg)

        assert len(result.eval_rewards) > 0, "No evaluation was performed"
        best_eval = max(result.eval_rewards)
        assert best_eval >= -400, (
            f"TD3 failed to reach -400 on Pendulum-v1. "
